...
// across all targets. Epics are skipped until the Tech Lead exists to
// decompose them; they are reported, never silently dropped.
export const decide = (
  snapshots: ReadonlyArray<TargetSnapshot>,
  config: CompanyConfig
): HeartbeatDecision => {
  const inFlight = snapshots.reduce((count, snapshot) => count + snapshot.wip.length, 0)
  let seats = Math.max(0, config.engineerParallelism - inFlight)
  ...
  return { claims, skippedEpics, inFlight }
}
...
export interface HeartbeatOptions {
  ...
  // the default until the Engineer pipeline exists, so the factory never
  // claims work it cannot do.
